...
function getUserInputs() {
    ...
    // Calculate loan details from user input
...
function calculateLoanDetails(loanAmount, totalPayments, intRate) {
    // loanAmount and intRate is being parsed from string to float numbers, it will ignore any letters inputted
    let parsedLoanAmount = parseFloat(loanAmount);
    let parsedIntRate = parseFloat(intRate);


    // totalPayments is parsed from a string into a integer
    let parsedTotalPayments = parseInt(totalPayments);


    // loanDetails will have the totalPrincipal, totalInterest, totalCost, and amortizationTable
    let loanDetails = {
    ...
    // M = monthly payment; P = principal loan amount; i = monthly interest rate; n = number of months to repay the loan    
    ...
    for (let payment = 1; payment <= totalPayments; payment++) {        
        ...
        // Push monthlyPaymentRow array to loanDetails and set monthlyPaymentRow to clear
        loanDetails.amortizationTable.push(monthlyPaymentRow);
        monthlyPaymentRow = [];

        // Decrement paymentsRemaining
        paymentsRemaining--;           
    }    
    return loanDetails
}
